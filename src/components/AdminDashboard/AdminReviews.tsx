...
const AllReviews: React.FC = () => {
  ...
  const [selectedReview, setSelectedReview] = useState<IReview | null>(null); // Estado para expandir solo una reseña

  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await fetch("https://proyectochecasa.onrender.com/reviews/");
        if (!response.ok) {
        ...
        {reviews.length > 0 ? (
          reviews.map((review) => (
            <li
              ...
              onClick={() => handleReviewClick(review)} // Configura la reseña seleccionada
            >
